...
type handlers struct {
	...
	adjustments *handler.LimitAdjustmentHandler
...
func New(db *gorm.DB, cfg config.Config) *gin.Engine {
	...
	registerLimitAdjustmentRoutes(protected, wired.adjustments)
...
func wire(db *gorm.DB, cfg config.Config) handlers {
	...
	adjustmentService := service.NewLimitAdjustmentService(db, adjustmentRepository, workerRepository, auditService)
	return handlers{
		...
		adjustments: handler.NewLimitAdjustmentHandler(adjustmentService),
		auth:        authService,
	}
}
